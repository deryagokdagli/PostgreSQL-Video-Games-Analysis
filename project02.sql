...
SELECT
    ...
    genre,
    total_na_sales,
    ROW_NUMBER() OVER (PARTITION BY genre ORDER BY total_na_sales DESC) AS rank_in_genre
FROM genre_sales
...
SELECT
    ...
    platform,
    total_sales,
    ROW_NUMBER() OVER (PARTITION BY publisher ORDER BY total_sales DESC) AS rn
